...
function lust.it(name, fn)
  ...
  if last_err_message then
    print(indent(lust.level + 1) .. last_err_message)
    last_err_message = nil
  end
  if err then
...
local function print_table(t)
  ...
  for i, v in ipairs(t) do
    arraypart = arraypart .. tostring(v) .. ", "
  end
  ...
  for k, v in pairs(t) do
    if type(k) == "number" and k % 1 == 0 then
      ...
      dictpart = dictpart .. "[" .. tostring(k) .. "]=" .. tostring(v) .. ", " 
    end
  end

  return "{" .. arraypart .. dictpart .. "}"
end
...
local function ofclass(t, T)
  if type(T) ~= "table" or not T.__traits then
    error("expected " .. tostring(T) .. " to be an Object class")
  end
  if not type(t) == "table" then
    return false,
      'expected ' .. tostring(t) .. " to be a " .. tostring(T),
      'expected ' .. tostring(t) .. " to not be a " .. tostring(T)
  end
  if not t.is then
    return false,
      'expected ' .. tostring(t) .. " to be a " .. tostring(T),
...
local paths = {
  ...
  equal = {
    test = function(v, x)
      local expected, actual = tostring(v), tostring(x)
      if type(v) == "table" then
        expected = print_table(v)
      end
      if type(x) == "table" then
        actual = print_table(x)
      end

      return strict_eq(v, x),
        'expected ' .. expected .. ' and ' .. actual .. ' to be exactly equal',
        'expected ' .. expected .. ' and ' .. actual .. ' to not be exactly equal'
    end
  },
  have = {
    test = function(v, x)
  ...
  fail = {
    test = function(f)
      ...
      return not ok,
        'expected ' .. tostring(f) .. ' to fail',
        'expected ' .. tostring(f) .. ' to not fail'
    end
  }
}
...
function lust.report_results()
  print(green..lust.passes.." tests PASSED"..normal)
  if lust.errors > 0 then
    print(red..lust.errors.." tests FAILED"..normal)
  else
